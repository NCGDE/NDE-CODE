...
class SDE(torch.nn.Module):
    ...
    def __init__(self,batch_size,state_size,brownian_size):
        super().__init__()
...
class RSDE(nn.Module):
    def __init__(self, batch_size, state_size            ):
        super().__init__()
        self.sde_=SDE(batch_size=batch_size,state_size=state_size,brownian_size=2)
    ...
    def forward(self, x):
        ...
        out=[]
        for i in x:
            ii=i.reshape(batch_size*num_nodes, output_dim)
...
class Seq2Seq(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.RSDE_=RSDE(batch_size=64*self.num_nodes,state_size=self.output_dim)
